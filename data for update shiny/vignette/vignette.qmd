...
data_dir <- here::here("data for update shiny", "Manuscript_data3.31")
...
library(mSigPlot)
# 建立别名以兼容可能存在的逻辑漏洞
if (!exists("plot_89")) plot_89 <- mSigPlot::plot_ID89
library(ggplot2)
library(scales)
library(future)
library(furrr)
...
source(here::here("data for update shiny", "vignette", "ppar.R"))

# Scale factor for 476-type plots (1.0 = default, 1.4 = 40% bigger)
plot476_scale <- 1.2

# Load helper functions for modular processing
source(here::here("data for update shiny", "vignette", "vhelpers.R"))
```
...
source(here::here("data for update shiny", "Manuscript_data3.31", "Mo_CAP9_analysis", "make_connection_table.R"))
}
...
source(here::here("data for update shiny", "code", "collapse_476_to_89.R"))
...
# 如果你在用并行计算（future/furrr），确保这个变量被分发到了子进程
...
plot_dir <- "figure/parallel_plots1"
...
for (i in seq_along(all_sig_data)) {
  ...
  res <- knitr::knit_child(here::here("data for update shiny", "vignette", "onesig.qmd"), envir = environment(), quiet = TRUE)
  cat(res, sep = "\n")
}
```
